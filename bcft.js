...
//	LET THE FUCKENING BEGIN!!1!1!
	async function GibMeMyWaterCell(){
		await waitFor(() => !!LoginResponse)
		modApi.hookFunction('LoginResponse', HOOK_PRIORITIES.AddBehaviour, (args, next) => {
			const ret = next(args);
	...
	async function RecordMyBlacklist(){
		...
		modApi.hookFunction('ChatRoomListUpdate', HOOK_PRIORITIES.AddBehaviour, (args, next) => {
			const ret = next(args);
			const action = args[0];
			const adding = args[1];
